...
#' @param probs (Required) Failure probabilities for each basic event. Can be a
...
#'   was already run).
#'
#' @return A single numeric value, or if \code{probs} has multiple rows (one per
...
quantify_prob = function(f, probs, truth_table = NULL) {
  ...
  if (!multi) {
    ...
    if (is.null(nms) || all(nms == "", na.rm = TRUE)) {
      ...
      if (length(probs) != length(fargs))
        stop("probs length must match number of basic events (", length(fargs), ").")
      names(probs) = fargs
    } else {
      probs = setNames(as.numeric(probs), nms)
      missing_events = setdiff(fargs, names(probs))
      if (length(missing_events) > 0L)
        stop("probs must contain values for all basic events. Missing: ",
             paste(missing_events, collapse = ", "))
    }
    if (any(probs < 0 | probs > 1, na.rm = TRUE))
      warning("Some probs are outside [0, 1]; result may not be a valid probability.")
  ...
  # Multi-scenario path: probs is matrix or data frame, one row per scenario
  if (is.data.frame(probs)) {
    missing_events = setdiff(fargs, colnames(probs))
    if (length(missing_events) > 0L)
      stop("probs must contain columns for all basic events. Missing: ",
           paste(missing_events, collapse = ", "))
    P = as.matrix(probs[, fargs, drop = FALSE])
  } else {
    if (is.null(colnames(probs)) || !all(fargs %in% colnames(probs)))
      stop("probs matrix must have column names matching basic events: ",
           paste(fargs, collapse = ", "))
    P = probs[, fargs, drop = FALSE]
  }
  P = matrix(as.numeric(P), nrow = nrow(P), ncol = ncol(P))
  if (any(P < 0 | P > 1, na.rm = TRUE))
    warning("Some probs are outside [0, 1]; results may not be valid probabilities.")
